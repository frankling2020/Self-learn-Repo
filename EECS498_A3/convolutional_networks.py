...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        # linear1
        self.params['W2'] = weight_scale * torch.randn(num_filters*(H//2)*(W//2), hidden_dim, dtype=dtype, device=device)
    ...
    def loss(self, X, y=None):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        scores, cache1 = Conv_ReLU_Pool.forward(X, W1, b1, conv_param, pool_param)
        scores, cache2 = Linear_ReLU.forward(scores, W2, b2)
        scores, cache3 = Linear.forward(scores, W3, b3)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        C, H, W = input_dims
        filter_sz = 3
        Hfilters = [C] + num_filters 
        for layer in range(self.num_layers-1):
            input_dim, output_dim = Hfilters[layer], Hfilters[layer+1]
            # conv
            w_name, b_name = 'W'+str(layer), 'b'+str(layer) 
            # initializer
            if weight_scale == 'kaiming':
                self.params[w_name] = kaiming_initializer(input_dim, output_dim, K=filter_sz, device=device, dtype=dtype)
            else:
                self.params[w_name] = weight_scale * torch.randn(output_dim, input_dim, filter_sz, filter_sz, dtype=dtype, device=device)
            self.params[b_name] = torch.zeros(output_dim, dtype=dtype, device=device)
            # batchnorm
            if self.batchnorm:
                gamma_name, beta_name = 'gamma'+str(layer), 'beta'+str(layer)
                self.params[gamma_name] = torch.ones(output_dim, device=device, dtype=dtype)
                self.params[beta_name] = torch.zeros(output_dim, device=device, dtype=dtype)
        
        # last linear layer
        input_dim = num_filters[-1] * (H // (2**len(max_pools))) * (W // (2**len(max_pools)))
        if weight_scale == 'kaiming':
            self.params['W'+str(self.num_layers-1)] = kaiming_initializer(input_dim, num_classes, relu=False, device=device, dtype=dtype)
        else:
            self.params['W'+str(self.num_layers-1)] = weight_scale * torch.randn(input_dim, num_classes, dtype=dtype, device=device)
        self.params['b'+str(self.num_layers-1)] = torch.zeros(num_classes, dtype=dtype, device=device)
        ################################################################
        #                      END OF YOUR CODE                        #
        ################################################################
    ...
    def loss(self, X, y=None):
        ...
        cache = None
        caches = []
        max_pool_layers = set(self.max_pools)
        # L-1 layers
        for layer in range(self.num_layers-1):
            w_name, b_name = 'W'+str(layer), 'b'+str(layer) 
        ...
        # last linear layer
        w_name, b_name = 'W'+str(self.num_layers-1), 'b'+str(self.num_layers-1)
        dout, grads[w_name], grads[b_name]= Linear.backward(dout, caches.pop())
        ...
        for layer in range(self.num_layers-2, -1, -1):
            ...
            if self.batchnorm:
                gamma_name, beta_name = 'gamma'+str(layer), 'beta'+str(layer)
                if layer in max_pool_layers:
                    dout, grads[w_name], grads[b_name], grads[gamma_name], grads[beta_name] = \
                ...
                if layer in max_pool_layers:
                    dout, grads[w_name], grads[b_name] = Conv_ReLU_Pool.backward(dout, cache)
                else:
                    dout, grads[w_name], grads[b_name] = Conv_ReLU.backward(dout, cache)
            grads[w_name] += 2*self.reg*self.params[w_name]
        #############################################################
        #                       END OF YOUR CODE                    #
        #############################################################
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    model = DeepConvNet(input_dims, [64, 128, 256], [0, 1, 2], weight_scale='kaiming', reg=3e-4, dtype=dtype, device=device)
    solver = Solver(
        model, data_dict,
        print_every=300,
        num_epochs=6, 
...
def kaiming_initializer(Din, Dout, K=None, relu=True, device='cpu',
    ...
    if K is None:
        ...
        ###################################################################
        # Replace "pass" statement with your code
        weight = torch.randn(Dout, Din, K, K, device=device, dtype=dtype) * ((gain/Din)**0.5) / K
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
    return weight
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            # Replace "pass" statement with your code
            x_mean = x.mean(dim=0)
            x_zero = x - x.mean(dim=0)
            x_var = torch.mean(x_zero ** 2, dim=0)
            std = torch.sqrt(x_var + eps)
            x_norm = x_zero / std
            out = gamma * x_norm + beta
            cache = (mode, x_norm, x_zero, std, gamma)
    ...
    def backward(dout, cache):
        ...
        #####################################################################
        # Replace "pass" statement with your code
        mode, x_norm, x_zero, std, gamma = cache 
        N, _ = dout.shape
        if mode == 'train':
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = x.shape
        out = x.permute([0, 2, 3, 1]).reshape(-1, C).contiguous()
        out_bn, cache = BatchNorm.forward(out, gamma, beta, bn_param)
        out = out_bn.reshape([N, H, W, C]).permute([0, 3, 1, 2]).contiguous()
        ################################################################
        #                       END OF YOUR CODE                       #
        ################################################################
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = dout.shape
        dout_bn = dout.permute([0, 2, 3, 1]).reshape(-1, C).contiguous()
        dx_bn, dgamma, dbeta = BatchNorm.backward_alt(dout_bn, cache)
        dx = dx_bn.reshape([N, H, W, C]).permute([0, 3, 1, 2]).contiguous()
        ##################################################################
        #                       END OF YOUR CODE                         #
        ##################################################################
