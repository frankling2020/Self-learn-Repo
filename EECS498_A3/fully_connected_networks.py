...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dhidden, grads['W2'], grads['b2'] = Linear.backward(dout, cache2)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for layer in range(1, self.num_layers):
            ...
            tmp, tmp_cache = Linear_ReLU.forward(tmp, w, b)
            caches.append(tmp_cache)
            if self.use_dropout:
                tmp, drop_cache = Dropout.forward(tmp, self.dropout_param)
                dropouts.append(drop_cache)
                
        w = self.params['W' + str(self.num_layers)]
        b = self.params['b' + str(self.num_layers)]
        scores, tmp_cache = Linear.forward(tmp, w, b)
        caches.append(tmp_cache)
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        dhidden, dw, db = Linear.backward(dout, caches.pop())
        ...
        for layer in range(self.num_layers-1, 0, -1):
            ...
            b_name = 'b' + str(layer)
            if self.use_dropout:
                dhidden = Dropout.backward(dhidden, dropouts.pop())
            dhidden, dw, db = Linear_ReLU.backward(dhidden, caches.pop())
            grads[w_name] = dw + self.reg*self.params[w_name]
            grads[b_name] = db
        ###########################################################
        #                   END OF YOUR CODE                      #
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(
		...
		data_dict,
		lr_decay=0.9,
		optim_config={'learning_rate':0.15}, 
		num_epochs=20,
		print_every=200,
		device='cuda'
    )
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Replace "pass" statement with your code
            mask = (torch.rand_like(x, device=x.device) < 1 - p)
            out = x * mask
            ##############################################################
            #                   END OF YOUR CODE                         #
            ##############################################################
