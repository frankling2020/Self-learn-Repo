...
class Encoder(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, activation, base_model=GCNConv, k: int = 2, skip=False):
        ...
        self.skip = skip
        if not self.skip:
            # if self.skip = False: the every layer is 2*out_channels
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor):
        if not self.skip:
            ...
            for i in range(1, self.k):
                # sum all the embeddings in the previous layer
                # the similar structure as ResNet
                # D = (I + M3) * (I + M2) * (I + M1) * A 
                u = sum(hs)
...
class GRACE(torch.nn.Module):
    ...
    def projection(self, z: torch.Tensor) -> torch.Tensor:
        # elu is better than relu except computing speed in test
        z = F.elu(self.fc1(z))
        return self.fc2(z)

    def sim(self, z1: torch.Tensor, z2: torch.Tensor):
        # can be changed into @staticmethods
        z1 = F.normalize(z1)
        z2 = F.normalize(z2)
    ...
    def semi_loss(self, z1: torch.Tensor, z2: torch.Tensor):
        ...
        # can be learned to simplify codes?
...
class LogReg(nn.Module):
    """
        one linear layer with xavier_uniform initialization
    """
